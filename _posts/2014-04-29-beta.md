...
$$P(x \le X_k \le x + \Delta x) = {n\choose k-1}{n\choose 1}x^{k-1}(1-x-\Delta x)^{n-k}\Delta x$$

所以我们可以得到$$X_k$$的概率密度函数为

$$\begin{align}f(x) &= \lim_{x \to 0} \frac{P(x\le X_k \le x+\Delta x)}{\Delta x}\\ &= {n \choose k-1}{n \choose 1}x^{k-1}(1-x)^{n-k}\\ &= \frac{n!}{(k-1)!(n-k)!}x^{k-1}(1-x)^{n-k}\\
...
上边这个分布其实就是一个三维形式的Dirichlet分布$$Dir(\alpha_1,\alpha_2,\alpha_3)$$.一般形式的Dirichlet分布定义如下

$$Dir(\vec p\|\vec \alpha)=\frac{\Gamma(\sum_{k=1}^K\alpha_k)}{\prod_{k=1}K\Gamma(\alpha_k)}\prod_{k=1}^Kp_k^{\alpha_k -1}$$

同Beta分布一样，Dirichlet分布也是共轭的
